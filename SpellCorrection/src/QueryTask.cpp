...
void QueryTask::queryIndex(const string& sin){
    auto head = _index[sin];//set<int>,包含该字母或者汉字的单词在词典的位置集合
    for(auto c : head){
...
string QueryTask::toJson(/*const string& rhs*/){
    ...
    while(total){
        MyResult pend = _resultque.top();
        _resultque.pop();
        word = pend._word;//使用json形式保存字符串，
        root.append(word);
        --total;
    }

    string json_file = writer.write(root);//转变为带有json格式的字符流
